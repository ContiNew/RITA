...
def pad_matrix_to_h_rows(matrix,h=4):
    # 현재 행과 열의 수를 확인
    rows, cols = matrix.shape
    # 행의 수가 4보다 작으면 0 벡터 추가
    if rows < h:
        # 부족한 행의 개수 계산
        rows_to_add = h - rows
        # (rows_to_add, cols) 크기의 0 벡터 생성
        zero_vectors = np.zeros((rows_to_add, cols))
        # 기존 매트릭스와 0 벡터를 수직으로 연결
...
def extract_patterns_from_note_start_w_flex_window(cm:ChartMatrix,pHeight:int=4, externalList:list=None)->list[dict]:
    ...
    while ps < len(matrix):
        ...
        if len(c_matrix)<pHeight:
            c_matrix = pad_matrix_to_h_rows(c_matrix,pHeight)
